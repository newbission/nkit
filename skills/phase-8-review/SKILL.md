...
|-------|----------|-------------|
| High | High | Can be used in other projects |
| Medium | Medium | Can be used in multiple places within same project |
| Low | Low | Used only for specific feature |
...
```typescript
// Bad: Requires modification for each extension
...
// Bad: Enumerated switch statements
switch (action.type) {
  case 'ADD': // ...
  case 'REMOVE': // ...
...
| - | DELETE /api/users/:id | Missing from design |
...
| Missing from design | In code, not in design | Add to design document |
